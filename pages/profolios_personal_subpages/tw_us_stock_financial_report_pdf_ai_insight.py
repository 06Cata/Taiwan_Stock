...
def get_financial_analysis(api_key: str, financial_text: str):
    ...
    user_prompt = f"""
你是一位專業說中文的財報分析師，詳細讀取公司PDF，根據三大財報資料(損益表、資產負債表、現金流量表)，進行以下面向的綜合財報分析
直接輸出下方條列（1.2...6)，不要加前言或角色定義說明
1 公司財報基本資料
- 公司名稱、美股或台股代號、財報涵蓋時間
2 高槓桿風險分析
...
6 上下游可以關注的個股，產業關係（列出個股美股或台股代號）
7 同業、競爭對手可以關注的個股，優劣勢比較（列出個股美股或台股代號）
8 分別列出市面上最新不同機構，對公司的估值，機構名稱、機構對此公司評價年月、目標價、評價、需注意點、產業前景(今日為{today_date}，不考慮財報，上網搜集高盛、摩根士丹利、摩根、瑞士信貸等最新評價)
